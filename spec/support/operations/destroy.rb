shared_examples 'has .destroy methods' do
  ...
  describe '.destroy' do
    it 'makes a DELETE request using the correct resource and resource id' do
      expect(Billymad::Request::Proxy).to receive(:delete).with(described_class, 999).and_return(true)
  ...
  describe '#destroy' do
    it 'makes a DELETE request using the correct resource and resource id' do
      ...
      expect(Billymad::Request::Proxy).to receive(:delete).with(described_class, 100).and_return(true)
