...
describe Billymad::ArticlePropertyValue do
  ...
  it "has correct .resource_name" do
    expect(described_class.send(:resource_name)).to eq('article-property-value')
  end

  it "has correct .plural_resource_name" do
    expect(described_class.send(:plural_resource_name)).to eq('article-property-values')
  end    
end