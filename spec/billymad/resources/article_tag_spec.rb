...
describe Billymad::ArticleTag do
  ...
  it "has correct .resource_name" do
    expect(described_class.send(:resource_name)).to eq('article-tag')
  end

  it "has correct .plural_resource_name" do
    expect(described_class.send(:plural_resource_name)).to eq('article-tags')
  end
end