...
describe Billymad::Request::Proxy do
  ...
  describe ".get" do
    it "builds request info with GET method and performs request" do
      Billymad::Request::Info.should_receive(:new).with(resource, method: :get, id: 123).and_return(request_info)
      Billymad::Request::Base.should_receive(:new).with(request_info).and_return(request_base)
  ...
  describe ".post" do
    it "builds request info with POST method and performs request" do
      Billymad::Request::Info.should_receive(:new).with(resource, method: :post, id: 123).and_return(request_info)
      Billymad::Request::Base.should_receive(:new).with(request_info).and_return(request_base)
  ...
  describe ".put" do
    it "builds request info with PUT method and performs request" do
      Billymad::Request::Info.should_receive(:new).with(resource, method: :put, id: 123).and_return(request_info)
      Billymad::Request::Base.should_receive(:new).with(request_info).and_return(request_base)
  ...
  describe ".delete" do
    it "builds request info with DELETE method and performs request" do
      Billymad::Request::Info.should_receive(:new).with(resource, method: :delete, id: 123).and_return(request_info)
      Billymad::Request::Base.should_receive(:new).with(request_info).and_return(request_base)
